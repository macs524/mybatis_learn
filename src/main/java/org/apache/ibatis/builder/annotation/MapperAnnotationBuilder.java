...
public class MapperAnnotationBuilder {
    ...
    public void parse() {
        ...
        // 除非已经解析过, 否则这个是不成立的, 因为XML解析完了之后会在前面加上namespace前缀
        if (!configuration.isResourceLoaded(resource)) {
            loadXmlResource();
            configuration.addLoadedResource(resource);
            assistant.setCurrentNamespace(type.getName());

            // 所以啊, mybatis 会对 xml进行解析, 随后也会对对应的接口进行解析, 只要接口有相关的注解的话.
            // 所以注解和XML是可以混用的, 简单的可以直接用注解
            parseCache();
            parseCacheRef(); //引用将替换原来的


            // 比较好奇的是, 如果只用接口的话, 是否还有resultMap的解析呢?
     ...
     *
     * 可以看到, 如果是包扫描, 那么需要保证和接口的同类目录下有同名的XML文件, 这个相当于限制了XML的位置和命名
     */
    ...
    /**
     * 解析缓存引用, 同样是配置在CacheNamespaceRef上
     */
    ...
    void parseStatement(Method method) {
        ...
        if (sqlSource != null) {
            ...
            Options options = method.getAnnotation(Options.class); // 通过Option设置
